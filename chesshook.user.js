...
(() => {
  ...
  const init = () => {
    ...
    vs.registerConfigValue({
      key: namespace + '_stockfishdepth',
      type: 'number',
      display: 'Stockfish Depth: ',
      description: 'Search depth (0 = use movetime instead)',
      value: 20,
      min: 0,
      max: 30,
      step: 1,
    ...
    vs.registerConfigValue({
      key: namespace + '_stockfishmovetime',
      type: 'number',
      display: 'Stockfish Move Time (ms): ',
      description: 'Time to think per move in ms (used when depth is 0)',
      value: 1000,
      min: 100,
      max: 30000,
      step: 100,
      showOnlyIf: () => !vs.queryConfigKey(namespace + '_legitmode') && vs.queryConfigKey(namespace + '_whichengine') === 'stockfish' && parseInt(vs.queryConfigKey(namespace + '_stockfishdepth')) === 0
    });

    vs.registerConfigValue({
      key: namespace + '_stockfishhash',
      type: 'number',
      display: 'Stockfish Hash (MB): ',
  ...
  const getEngineMove = () => {
    ...
    if (vs.queryConfigKey(namespace + '_whichengine') === 'betafish') {
      ...
      const sfDepth = parseInt(vs.queryConfigKey(namespace + '_stockfishdepth'));
      if (sfDepth > 0) {
        stockfishWorker.postMessage('go depth ' + sfDepth);
      } else {
        const sfMovetime = vs.queryConfigKey(namespace + '_stockfishmovetime') || 1000;
        stockfishWorker.postMessage('go movetime ' + sfMovetime);
      }
    }
  }
